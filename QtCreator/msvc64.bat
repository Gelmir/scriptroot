...
SET "PATH=%BUILDROOT%\Qt\Qt5_x64_full\bin;C:\_\Python27;C:\Program Files\7-Zip;%BUILDROOT%\jom;%PATH%"
IF EXIST %SOURCEROOT%\Qt RD /S /Q %SOURCEROOT%\Qt
MD %SOURCEROOT%\Qt
CD %SOURCEROOT%\Qt
"C:\Program Files\7-Zip\7z.exe" x %ARCHIVES%\QT-5.1.0.7z -o%SOURCEROOT%\Qt qtbase/
IF EXIST %SOURCEROOT%\QtCreator RD /S /Q %SOURCEROOT%\QtCreator
...
IF EXIST %SOURCEROOT%\mkspecs RD /S /Q %SOURCEROOT%\mkspecs
"C:\Program Files\7-Zip\7z.exe" x -o%SOURCEROOT% %ARCHIVES%\QT-4.8.5.7z qtbase/mkspecs
patch --binary -p1 -Nsfi %SCRIPTROOT%\Qt\patches\msvc_Qt5.diff -d %SOURCEROOT%\
:: ignore errors here
:: IF ERRORLEVEL 1 GOTO FAIL
...
FOR %%X IN (assistant qt designer linguist qmake qml) DO (
    ...
    COPY /Y %BUILDROOT%\Qt\Qt5_x64_full\doc\qch\%%X.qch %INSTALL_ROOT%\share\doc\%%X\
)
:: Copy SSL libs
...
FOR %%X IN (bearer codecs phonon_backend) DO (
	XCOPY /E /Y /Q /I %BUILDROOT%\Qt\Qt5_x64_full\plugins\%%X %INSTALL_ROOT%\bin\%%X
)
:: Purge .lib files
