...
MD qtcb
CD qtcb
qmake -config release -r ../qtcreator.pro "QT_PRIVATE_HEADERS = T:/sources/Qt/include " "CONFIG += warn_off ltcg mmx sse sse2" "CONFIG -= 3dnow"
...
qmake -config release -r ../qtcreator.pro "QT_PRIVATE_HEADERS = T:/sources/Qt/include " "CONFIG += warn_off msvc_mp ltcg mmx sse sse2" "CONFIG -= 3dnow"
IF ERRORLEVEL 1 GOTO FAIL
...
FOR %%X IN (assistant qt designer linguist qmake qml) DO (
    ...
    COPY /Y %BUILDROOT%\Qt\Qt64\doc\qch\%%X.qch %INSTALL_ROOT%\share\doc\%%X\
)
:: Copy SSL libs
...
FOR %%X IN (bearer codecs phonon_backend) DO (
	XCOPY /E /Y /Q /I %BUILDROOT%\Qt\Qt64\plugins\%%X %INSTALL_ROOT%\bin\%%X
)
:: Purge .lib files
