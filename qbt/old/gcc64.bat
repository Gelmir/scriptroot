...
patch --binary -p1 -Nsfi %SCRIPTROOT%\qbt\patches\gcc64.patch
IF ERRORLEVEL 1 GOTO FAIL
SET "PATH=%BUILDROOT%\Qt\Qt64_G\bin;%PATH%"
CD .\src
...
MD build
CD build
qmake -config release -r ../qbittorrent.pro "CONFIG += strace_win warn_off rtti mmx sse sse2" "CONFIG -= 3dnow"
IF ERRORLEVEL 1 GOTO FAIL
mingw32-make -j4
IF ERRORLEVEL 1 GOTO FAIL
...
FOR %%X IN (QtCore4.dll QtGui4.dll QtNetwork4.dll QtXml4.dll) DO (
    COPY /Y %BUILDROOT%\Qt\Qt64_G\bin\%%X %INST_DIR%\
)
:: Only qico4.dll is required
XCOPY /Y /Q /I %BUILDROOT%\Qt\Qt64_G\plugins\imageformats\qico4.dll %INST_DIR%\plugins\imageformats\
:: Use never Qt translations if possible
FOR /F "usebackq" %%X IN (`DIR /B "%SOURCEROOT%\qbittorrent\src\qt-translations\"`) DO (
  IF EXIST "%BUILDROOT%\Qt\Qt64_G\translations\%%X" (
    COPY /Y "%BUILDROOT%\Qt\Qt64_G\translations\%%X" "%SOURCEROOT%\qbittorrent\src\qt-translations\"
  )
)
...
sed -i -e "s/^\x0C\(.*\)/\1/" %INST_DIR%\LICENSE.txt
