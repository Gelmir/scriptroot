...
SET QT_VER=
IF DEFINED LT_MOVED (
	MOVE /Y %BUILDROOT%\libtorrent\libtorrent64_G %BUILDROOT%\libtorrent\libtorrent64_0_G
    MOVE /Y %BUILDROOT%\libtorrent\libtorrent64_1_G %BUILDROOT%\libtorrent\libtorrent64_G
	SET LT_MOVED=
)
GOTO END
:: Control git branhes
...
IF DEFINED TAG_RELEASE (
  ...
  :: Move libtorrent
  MOVE /Y %BUILDROOT%\libtorrent\libtorrent64_G %BUILDROOT%\libtorrent\libtorrent64_1_G
  MOVE /Y %BUILDROOT%\libtorrent\libtorrent64_0_G %BUILDROOT%\libtorrent\libtorrent64_G
  SET LT_MOVED=1
) ELSE (
  IF DEFINED NO_TAINT (
...
CD build
IF NOT DEFINED LOG (
  qmake -config release -r ../qbittorrent.pro "CONFIG += strace_win warn_off rtti mmx sse sse2" "CONFIG -= 3dnow ltcg"
  IF ERRORLEVEL 1 GOTO FAIL
) ELSE (
  qmake -config release -r ../qbittorrent.pro "CONFIG += strace_win warn_off rtti mmx sse sse2" "CONFIG -= 3dnow ltcg" "DEFINES += TORRENT_DISK_STATS TORRENT_LOGGING"
  IF ERRORLEVEL 1 GOTO FAIL
)
mingw32-make -j4
IF ERRORLEVEL 1 GOTO FAIL
...
IF %QT_VER% == 5 (
  FOR %%X IN (Qt5Core.dll Qt5Gui.dll Qt5Network.dll Qt5Widgets.dll Qt5Xml.dll) DO (
    COPY /Y %BUILDROOT%\Qt\Qt5_gcc64_qbt\bin\%%X %INST_DIR%\
  )
  :: Only qico4.dll is required
  XCOPY /Y /Q /I %BUILDROOT%\Qt\Qt5_gcc64_qbt\plugins\imageformats\qico.dll %INST_DIR%\plugins\imageformats\
  :: Not sure if needed
  XCOPY /Y /Q /I /E %BUILDROOT%\Qt\Qt5_gcc64_qbt\plugins\platforms %INST_DIR%\plugins\platforms
  ...
  FOR /F "usebackq" %%X IN (`DIR /B "%SOURCEROOT%\qbittorrent\src\qt-translations\" ^| perl -pe "s/^.*?_(.*)/\1/"`) DO (
    IF EXIST "%BUILDROOT%\Qt\Qt5_gcc64_qbt\translations\qt_%%X" (
      COPY /Y "%BUILDROOT%\Qt\Qt5_gcc64_qbt\translations\qt_%%X" "%SOURCEROOT%\qbittorrent\src\qt-translations\"
    )
    IF EXIST "%BUILDROOT%\Qt\Qt5_gcc64_qbt\translations\qtbase_%%X" (
      COPY /Y "%BUILDROOT%\Qt\Qt5_gcc64_qbt\translations\qtbase_%%X" "%SOURCEROOT%\qbittorrent\src\qt-translations\"
    )
  )
  ...
  FOR %%X IN (QtCore4.dll QtGui4.dll QtNetwork4.dll QtXml4.dll) DO (
    COPY /Y %BUILDROOT%\Qt\Qt4_gcc64_qbt\bin\%%X %INST_DIR%\
  )
  :: Only qico4.dll is required
  XCOPY /Y /Q /I %BUILDROOT%\Qt\Qt4_gcc64_qbt\plugins\imageformats\qico4.dll %INST_DIR%\plugins\imageformats\
  :: Use newer Qt translations if possible
  FOR /F "usebackq" %%X IN (`DIR /B "%SOURCEROOT%\qbittorrent\src\qt-translations\"`) DO (
    IF EXIST "%BUILDROOT%\Qt\Qt4_gcc64_qbt\translations\%%X" (
      COPY /Y "%BUILDROOT%\Qt\Qt4_gcc64_qbt\translations\%%X" "%SOURCEROOT%\qbittorrent\src\qt-translations\"
    )
  )
  XCOPY /Y /Q /I %SOURCEROOT%\qbittorrent\src\qt-translations\qt_* %INST_DIR%\translations\
)
