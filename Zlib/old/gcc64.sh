...
# Editing CFLAGS
sed -i -e "s/\(CFLAGS = \$(LOC) \)-O3\( -Wall\)/\1\2 -mtune=generic -mmmx -msse -msse2 -pipe -fstack-protector-all -fno-exceptions -O3 -fomit-frame-pointer -fpredictive-commoning -finline-small-functions -finline-functions -m64/" -e "s/\(ASFLAGS = \).*/\1\$(CFLAGS)/" -e "s/\(LDFLAGS = \$(LOC)\)/\1 -fstack-protector-all -Wl,-s -Wl,-O1 -Wl,--as-needed -Wl,--nxcompat -Wl,--dynamicbase/" ./win32/Makefile.gcc
