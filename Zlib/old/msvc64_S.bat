...
SET "INST_DIR=%BUILDROOT%\Zlib\Zlib64_S"
...
sed -b -e "s/\(^ASFLAGS = \).*\(\$(LOC)\)/\1\2/" -e "s/\(^AS = \).*/\1ml64/" -e "s/\(^CFLAGS  = \).*\(\$(LOC)\)/\1 -nologo -MT -O2 -W3 -favor\:blend -GL -GR- -Y- -MP -EHs-c- \2/" -e "s/\(^LDFLAGS = \).*/\1-nologo -incremental\:no -opt\:ref -opt\:icf=5 -ltcg/" -e "s/\(^ARFLAGS = .*\)/\1 -ltcg/" < .\win32\Makefile.msc > .\win32\Makefile.msc.%SEDEXT%
