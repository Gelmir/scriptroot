...
"/c/Program Files/7-Zip/7z.exe" x /t/_compressed_sources/OpenSSL-1.0.1c.7z -o"${G_SOURCEROOT}"/OpenSSL
...
# Seems to NOT use SHARED_LDFLAGS in any way - dump everyhting in CFLAGS
 _CFLAGS_="-O2 -mtune=generic -mmmx -msse -msse2 -fomit-frame-pointer -fpredictive-commoning -pipe -m64 -fno-exceptions -finline-small-functions -finline-functions -fstack-protector-all -Wl,-O1 -Wl,--as-needed -Wl,-s -shared-libgcc -Wl,--nxcompat -Wl,--dynamicbase"
_LDFLAGS_="${_CFLAGS_}"

perl Configure mingw64 threads shared zlib -I"${G_BUILDROOT}/Zlib/Zlib64_G/include" -L"${G_BUILDROOT}/Zlib/Zlib64_G/lib" --prefix="${INST_DIR}"
