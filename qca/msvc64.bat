...
SET "PATH=%BUILDROOT%\Qt\Qt5_x64_full\bin;%BUILDROOT%\OpenSSL\OpenSSL64\bin;%BUILDROOT%\jom;%PATH%"
...
jom -j4
IF ERRORLEVEL 1 GOTO FAIL
SET "PATH=%SOURCEROOT%\qca\build\src;%BUILDROOT%\icu\icu64\bin64;%PATH%"
jom -j1 test
IF ERRORLEVEL 1 GOTO FAIL
jom -j1 install
IF ERRORLEVEL 1 GOTO FAIL
